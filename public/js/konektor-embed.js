...
(function () {
    ...
    // ── VID cookie (shared with standalone form page) ──
    ...
    // ── Pixel endpoint: server-side page_load CAPI ──
    var pixelUrl = actionUrl.replace(/\/submit\/?$/, '/pixel');
    var clickId  = '';
    try {
        var qs = new URLSearchParams(window.location.search);
        clickId = qs.get('click_id') || qs.get('clickid') || '';
    } catch (e) {}
    var pixelQs = '?url=' + encodeURIComponent(window.location.href);
    if (clickId) pixelQs += '&click_id=' + encodeURIComponent(clickId);
    if (vid)     pixelQs += '&_vid='     + encodeURIComponent(vid);
    fetch(pixelUrl + pixelQs, { mode: 'cors' }).catch(function(){});
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(function(res) {
                if (res.success) {
                    ...
                    // Browser-side pixel events (form_submit)
