...
class Konektor_Analytics {
    ...
    /**
     * Normalize a date range. Returns [date_from, date_to] as 'Y-m-d' strings.
     */
    public static function resolve_date_range( $days = 30, $date_from = null, $date_to = null ) {
        $to   = ( $date_to   && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $date_to ) )   ? $date_to   : date( 'Y-m-d' );
        $from = ( $date_from && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $date_from ) ) ? $date_from : date( 'Y-m-d', strtotime( "-{$days} days", strtotime( $to ) ) );
        if ( $from > $to ) { $tmp = $from; $from = $to; $to = $tmp; }
        return [ $from, $to ];
    }

    public static function get_daily( $days = 30, $campaign_id = null, $date_from = null, $date_to = null ) {
        global $wpdb;
        [ $from, $to ] = self::resolve_date_range( $days, $date_from, $date_to );
        ...
        while ( $cur <= $end ) {
            $d     = date( 'Y-m-d', $cur );
            $out[] = (object) [ 'date' => $d, 'leads' => isset( $map[ $d ] ) ? $map[ $d ] : 0 ];
            $cur   = strtotime( '+1 day', $cur );
        }
        return $out;
    }

    public static function get_summary( $date_from = null, $date_to = null, $days = 30 ) {
        global $wpdb;
        [ $from, $to ] = self::resolve_date_range( $days, $date_from, $date_to );
        ...
        return [
            'total_leads'  => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_l WHERE DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'new_leads'    => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_l WHERE status='new' AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'purchased'    => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_l WHERE status='purchased' AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'double_leads' => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_l WHERE is_double=1 AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'blocked'      => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_l WHERE status='blocked' AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'page_loads'   => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_e WHERE event_type='page_load' AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
            'form_submits' => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $table_e WHERE event_type='form_submit' AND DATE(created_at) BETWEEN %s AND %s", $from, $to ) ),
        ];
    }
