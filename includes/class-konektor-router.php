...
class Konektor_Router {
    ...
    private static function handle_pixel_event( $campaign ) {
        ...
        $lead_data  = [
            ...
            '_vid'       => sanitize_text_field( $_GET['_vid'] ?? '' ),
    ...
    private static function handle_submit( $campaign ) {
        ...
        if ( ! Konektor_Helper::rate_limit( 'submit_' . $ip, 20, 60 ) ) {
            ...
            if ( strpos( $ct, 'application/json' ) !== false ) {
                header( 'Content-Type: application/json; charset=utf-8' );
                echo wp_json_encode( [ 'success' => false, 'message' => 'Terlalu banyak permintaan. Coba lagi sebentar.' ] );
            } else {
                wp_die( 'Terlalu banyak permintaan. Coba lagi sebentar.', 429 );
            }
            exit;
        }
        ...
        if ( $origin ) {
            ...
            if ( ! $ok ) {
                foreach ( $allowed as $d ) {
                    if ( $host === $d || ( strlen( $host ) > strlen( $d ) && substr( $host, -( strlen( $d ) + 1 ) ) === '.' . $d ) ) { $ok = true; break; }
                }
            }
            if ( $ok ) {
                header( 'Access-Control-Allow-Origin: ' . esc_url_raw( $origin ) );
        ...
        $ct       = $_SERVER['CONTENT_TYPE'] ?? '';
        $is_embed = strpos( $ct, 'application/json' ) !== false;
    ...
    private static function serve_asset( $file ) {
        ...
        $mime = substr( $file, -4 ) === '.css' ? 'text/css' : 'application/javascript';
    ...
    private static function render_cs_panel() {
        ...
        if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
            ...
            // Rate limit CS panel mutations: 60 per minute per token
            if ( ! Konektor_Helper::rate_limit( 'cs_' . md5( $token ), 60, 60 ) ) {
                status_header( 429 );
                echo wp_json_encode( [ 'success' => false, 'message' => 'Terlalu banyak permintaan.' ] );
                exit;
            }
    ...
    private static function handle_cs_ajax( $act, $operator ) {
        ...
        if ( $act === 'get_leads' ) {
            ...
            $where  = 'l.operator_id = %d AND c.type = \'form\'';
            ...
            $total = (int) $wpdb->get_var( $wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}konektor_leads l
                 JOIN {$wpdb->prefix}konektor_campaigns c ON c.id = l.campaign_id
                 WHERE $where", ...$params
            ) );
    ...
    private static function parse_input() {
        $ct = $_SERVER['CONTENT_TYPE'] ?? '';
        if ( strpos( $ct, 'application/json' ) !== false ) {
            // Cap at 64 KB to prevent DoS via oversized payload
        ...
        foreach ( $limits as $field => $max ) {
            if ( isset( $data[ $field ] ) ) {
                $data[ $field ] = substr( (string) $data[ $field ], 0, $max );
            }
        }
        return $data;
    }
