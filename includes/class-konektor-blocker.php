...
class Konektor_Blocker {
    ...
    public static function block( $data, $operator_id = null ) {
        global $wpdb;
        $block_by = sanitize_text_field( $data['block_by'] ?? 'both' );
        $ip       = sanitize_text_field( $data['ip_address'] ?? '' );
        $cookie   = sanitize_text_field( $data['cookie_id'] ?? '' );

        $save_ip    = in_array( $block_by, [ 'ip', 'both' ], true ) ? $ip : null;
        $save_cookie = in_array( $block_by, [ 'cookie', 'both' ], true ) ? $cookie : null;
