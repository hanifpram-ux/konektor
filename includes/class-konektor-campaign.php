...
class Konektor_Campaign {
    ...
    public static function get_embed_code( $campaign ) {
        ...
        $js = "<script>(function(){\n"
            ...
            . ( ! empty( $meta_cfg_js['form_submit_event'] ) ? "          if(window.fbq)fbq('track'," . wp_json_encode( $meta_cfg_js['form_submit_event'] ) . ");\n" : '' )
            . ( ! empty( $tiktok_cfg_js['form_submit_event'] ) ? "          if(window.ttq)ttq.track(" . wp_json_encode( $tiktok_cfg_js['form_submit_event'] ) . ");\n" : '' )
            . "          if(window.gtag&&" . wp_json_encode( ! empty( $google_cfg_js['conversion_id'] ) ? $google_cfg_js['conversion_id'] : '' ) . "){\n"
            . "            gtag('event','conversion',{'send_to':'AW-" . esc_js( ! empty( $google_cfg_js['conversion_id'] ) ? $google_cfg_js['conversion_id'] : '' ) . "/" . esc_js( ! empty( $google_cfg_js['form_submit_label'] ) ? $google_cfg_js['form_submit_label'] : '' ) . "'});\n"
            . "          }\n"
            . "        }\n"
