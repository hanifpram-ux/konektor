...
class Konektor_Campaign {
    ...
    private static function get_pixel_ping( $campaign ) {
        ...
        return "<script>(function(){"
             ...
             . "window['{$dedup_key}']=1;"
             . "var u='" . $pixel_url . "';"
             . "var qs='?url='+encodeURIComponent(location.href);"
             . "var vid=document.cookie.match('(?:^|;)\\s*konektor_vid=([^;]*)');"
             . "if(vid)qs+='&_vid='+encodeURIComponent(vid[1]);"
             . "var c=document.location.search.match(/[?&](click_id|clickid)=([^&]*)/);"
             . "if(c)qs+='&click_id='+encodeURIComponent(c[2]);"
             . "fetch(u+qs,{method:'GET',credentials:'include'}).catch(function(){});"
             . "})();</script>\n";
    }
    ...
    public static function get_wa_embed_code( $campaign ) {
        ...
        return "<!-- Konektor Link: {$camp_name} -->\n"
             . $meta_html . $google_html . $tiktok_html . $snack_html . $pixel_ping . $css
             . "<div class=\"knk-wa\">\n"
             . $header
    ...
    public static function get_embed_code( $campaign ) {
        ...
        $vid_cookie_js = self::get_cookie_js( $campaign );

        return "<!-- Konektor Form: " . esc_html( $campaign->name ) . " -->\n"
             . $meta_html . $google_html . $tiktok_html . $snack_html . $pixel_ping . $css
             . "<div class=\"knk-form-wrap\">\n"
             . "  <form class=\"knk-form\" method=\"POST\" action=\"" . esc_attr( $submit_url ) . "\">\n"
             . "    <input type=\"hidden\" name=\"_vid\" id=\"knk-vid-" . esc_attr( $campaign->id ) . "\" value=\"\">\n"
             . "    <input type=\"hidden\" name=\"source_url\" value=\"" . esc_attr( home_url( $_SERVER['REQUEST_URI'] ?? '/' ) ) . "\">\n"
             . "    <input type=\"hidden\" name=\"referrer\" value=\"" . esc_attr( sanitize_text_field( $_SERVER['HTTP_REFERER'] ?? '' ) ) . "\">\n"
             . "    <input type=\"hidden\" name=\"click_id\" value=\"" . esc_attr( sanitize_text_field( $_GET['click_id'] ?? $_GET['clickid'] ?? '' ) ) . "\">\n"
             . $header . $fh
             ...
             . "<script>(function(){\n"
             . $vid_cookie_js
             . "  var el=document.getElementById('knk-vid-" . esc_js( $campaign->id ) . "');\n"
             . "  if(el)el.value=_knkVid();\n"
             . "})()</script>\n"
             . "<!-- End Konektor Form -->";
    }
