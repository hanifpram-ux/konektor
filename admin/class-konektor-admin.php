...
class Konektor_Admin {
    ...
    public static function page_analytics() {
        ...
        [ $date_from, $date_to ] = Konektor_Analytics::resolve_date_range( $days, $date_from, $date_to );
        $is_custom = isset( $_GET['date_from'] ) || isset( $_GET['date_to'] );

        $summary      = Konektor_Analytics::get_summary( $date_from, $date_to, $days );
        $per_campaign = Konektor_Analytics::get_per_campaign();
        $per_operator = Konektor_Analytics::get_per_operator();
        $daily        = Konektor_Analytics::get_daily( $days, null, $date_from, $date_to );
        $range_label  = ( $date_from === $date_to ) ? $date_from : $date_from . ' s/d ' . $date_to;
    ...
    public static function page_settings() {
        ...
        $test_logs = Konektor_Logger::get_logs( [ 'per_page' => 20, 'page' => 1 ] );
    ...
    public static function page_log() {
        ...
        if ( isset( $_POST['knk_clear_log'] ) && check_admin_referer( 'knk_clear_log' ) ) {
            $ok = Konektor_Logger::clear();
            if ( $ok ) {
                wp_safe_redirect( admin_url( 'admin.php?page=konektor-log&cleared=1' ) );
                exit;
            } else {
                echo '<div class="notice notice-error"><p>Gagal menghapus log. Cek permission database.</p></div>';
            }
        }
        if ( isset( $_GET['cleared'] ) ) {
            echo '<div class="notice notice-success"><p>Semua log berhasil dihapus.</p></div>';
        }
    ...
    public static function ajax_test_pixel() {
        ...
        if ( $platform === 'meta' ) {
            ...
            $cfg = $pixel_cfg['google'] ?? [];
            if ( empty( $cfg['conversion_id'] ) ) {
                wp_send_json_error( [ 'message' => 'Conversion ID Google wajib diisi.' ] );
            }
            $label_map = [
            ...
            $payload = [
                'conversions' => [[
                    'gclid'                => 'TESTGCLID123',
                    'conversion_action'    => "customers/{$cfg['conversion_id']}/conversionActions/{$label}",
                    'conversion_date_time' => current_time( 'Y-m-d H:i:sP' ),
                    'conversion_value'     => $value,
                    'currency_code'        => $currency,
                ]],
            ];
            Konektor_Logger::log( 'Google Ads CAPI', $event_type, 'google_ads_conversion_upload', $payload, [
                'body' => wp_json_encode( [ 'note' => 'gclid captured, conversion logged locally. Configure OAuth2 for live upload.' ] ),
                'code' => 200,
            ] );
            wp_send_json_success( [
                'ok'      => true,
                'code'    => 200,
                'body'    => wp_json_encode( [ 'payload' => $payload, 'note' => 'Google Ads CAPI memerlukan OAuth2 untuk upload live. Payload di atas siap dikirim.' ] ),
                'event'   => $event_type,
    ...
    public static function ajax_debug_campaign() {
        ...
        if ( $test_code && ! empty( $meta_cfg['token'] ) && ! empty( $meta_cfg['pixel_id'] ) ) {
            ...
            $event_name = $meta_cfg['form_submit_event'] ?? '';
            $lead_data  = [
...
function refreshPreview(){
  ...
  var colors={modern:'#2563eb',classic:'#dc2626',classic_flat:'#FF5000',minimal:'#111',card:'#2563eb',gradient:'#fff',rose:'#e11d48',forest:'#16a34a',sunset:'#ee5a24',ocean:'#48cae4'};
  var bg={modern:'#fff',classic:'#fff',classic_flat:'#b7d09a',minimal:'#fff',card:'#f8fafc',gradient:'#1e3a5f',rose:'#fff1f2',forest:'#f0fdf4',sunset:'#fff3e0',ocean:'#e0f7fa'};
  var btnc=colors[tpl]||'#2563eb';
  var bgg=bg[tpl]||'#fff';
  var isCF=(tpl==='classic_flat');
  ...
  fields.forEach(function(f){
    ...
    h+='<label style="display:block;font-size:11px;font-weight:'+(isCF?'bold':'600')+';margin-bottom:4px">'+esc(f.label)+'</label>';
    if(f.type==='textarea'){
      h+='<textarea style="width:100%;padding:7px;border:'+(isCF?'1px solid #ccc':'1.5px solid #cbd5e1')+';border-radius:'+(isCF?'3px':'6px')+';font-size:12px;resize:none;height:60px" placeholder="'+esc(f.label)+'"></textarea>';
    }else{
      h+='<input type="'+esc(f.type)+'" style="width:100%;'+(isCF?'height:32px;padding:0 8px;':'padding:7px 10px;')+'border:'+(isCF?'1px solid #ccc':'1.5px solid #cbd5e1')+';border-radius:'+(isCF?'3px':'6px')+';font-size:12px" placeholder="'+esc(f.label)+'">';
    }
    h+='</div>';
  });
  h+='<button style="width:100%;'+(isCF?'height:44px;padding:0 20px;':'padding:10px;')+'background:'+btnc+';color:#fff;border:none;border-radius:'+(isCF?'3px':'7px')+';font-size:14px;font-weight:600;cursor:default">'+esc(lbl)+'</button>';
  h+='</div>';
...
$(document).on('change','#tp-platform',function(){
  var v=$(this).val();
  $('#tp-fields-meta,#tp-fields-tiktok,#tp-fields-snack,#tp-fields-google').hide();
  $('#tp-fields-'+v).show();
  $('#tp-result').hide();
  $('#tp-status').text('');
});
...
$(document).on('click','#tp-send-btn',function(){
  ...
  if(platform==='meta'){
    ...
    var convId=$('#tp-google-conversion-id').val().trim();
    var convLabel=$('#tp-google-conversion-label').val().trim();
    if(!convId){$status.text('Conversion ID wajib diisi.').css('color','var(--err)');return;}
    var gCfg={conversion_id:convId};
    if(convLabel)gCfg.form_submit_label=convLabel;
    pixelCfg={google:gCfg};
  }
