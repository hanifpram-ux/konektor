    ...
    public static function ajax_save_campaign() {
        ...
        foreach ( ['form_config','thanks_page_config','pixel_config','operators','allowed_domains'] as $key ) {
            if ( ! empty( $data[ $key ] ) && is_string( $data[ $key ] ) ) {
                $data[ $key ] = json_decode( wp_unslash( $data[ $key ] ), true ) ?: [];
            }
        }
...
.knk-embed-box{display:none;margin-top:6px;width:100%;padding:9px 11px;font-family:monospace;font-size:11px;background:#fff;border:1.5px solid var(--p-bd);border-radius:6px;resize:none;height:72px}
.knk-embed-box.open{display:block}
/* field builder */
...
.knk-field-row-name{font-weight:600;font-size:13px}
.knk-field-row-type{font-size:10px;color:var(--g500);background:var(--g200);padding:1px 6px;border-radius:3px;text-transform:uppercase}
...
function refreshPreview(){
  ...
  $('#knk-field-list .knk-field-row').each(function(){
    ...
    if($r.find('.knk-field-enable-cb').is(':checked')){
      fields.push({
        label:$r.find('.knk-field-row-name').text(),
...
function buildCampaignPayload($form,id){
  ...
  $form.serializeArray().forEach(function(item){
    ...
    var mPix=k.match(/^pixel_config\[(meta|google|tiktok)\]\[(\w+)\]$/);
    if(mPix){
      if(mPix[1]==='meta')pixelMeta[mPix[2]]=v;
      else if(mPix[1]==='google')pixelGoogle[mPix[2]]=v;
      else pixelTiktok[mPix[2]]=v;
      return;
    }
  ...
  p.pixel_config=JSON.stringify({meta:pixelMeta,google:pixelGoogle,tiktok:pixelTiktok});
