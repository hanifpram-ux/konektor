(function($) {
    ...
    $(document).on('click', '.knk-ptab[data-ptab]', function() {
        ...
        var $wrap = $(this).closest('.knk-card__body');
        $wrap.find('.knk-ptab[data-ptab]').removeClass('active');
        $(this).addClass('active');
        $wrap.find('.knk-pcontent[data-ptab]').removeClass('active');
    ...
    function buildCampaignPayload($form, id) {
        ...
        raw.forEach(function(item) {
            ...
            // pixel_config[meta|google|tiktok|snack][field]
            var mPixel = k.match(/^pixel_config\[(meta|google|tiktok|snack)\]\[(\w+)\]$/);
            if (mPixel) {
                if (mPixel[1] === 'meta')   pixelMeta[mPixel[2]]   = v;
                if (mPixel[1] === 'google') pixelGoogle[mPixel[2]] = v;
                if (mPixel[1] === 'tiktok') pixelTiktok[mPixel[2]] = v;
                if (mPixel[1] === 'snack')  pixelSnack[mPixel[2]]  = v;
                return;
            }
            // operators[id][field]
        ...
        payload.pixel_config = JSON.stringify({ meta: pixelMeta, google: pixelGoogle, tiktok: pixelTiktok, snack: pixelSnack });
