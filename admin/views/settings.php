...
<div class="knk-stab-content knk-hidden" id="stab-test-pixel">
  ...
  <div class="knk-card">
    ...
    <div class="knk-card-body">
      ...
      <div class="knk-g2" style="margin-bottom:14px">
        <div class="knk-field">
          ...
          <select id="tp-platform" class="knk-select">
            ...
            <option value="google">Google Ads / GTM / GA4</option>
          </select>
        </div>
        <div class="knk-field">
      ...
      <!-- Google Fields -->
      <div id="tp-fields-google" style="display:none">
        <div class="knk-g2" style="margin-bottom:14px">
          <div class="knk-field">
            <label class="knk-label">Conversion ID <span class="knk-req">*</span></label>
            <input type="text" id="tp-google-conversion-id" class="knk-input" placeholder="123456789">
          </div>
          <div class="knk-field">
            <label class="knk-label">Conversion Label</label>
            <input type="text" id="tp-google-conversion-label" class="knk-input" placeholder="Label konversi">
          </div>
        </div>
      </div>
      ...
      <!-- Log Viewer -->
      <div style="margin-top:24px;border-top:1px solid var(--g200);padding-top:18px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
          <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:var(--g500);">Riwayat Test &amp; Log API</div>
          <a href="<?php echo esc_url( admin_url( 'admin.php?page=konektor-log' ) ); ?>" class="knk-btn knk-btn-ghost knk-btn-sm">Lihat Semua Log</a>
        </div>
        <?php if ( empty( $test_logs ) ) : ?>
        <div style="text-align:center;padding:24px;color:var(--g400);font-size:13px;">Belum ada log.</div>
        <?php else : ?>
        <div class="knk-table-wrap">
        <table class="knk-table" style="font-size:12px">
          <thead>
            <tr>
              <th>Platform</th>
              <th>Event</th>
          ...
          <tbody>
            ...
            <tr>
              <td style="font-weight:500"><?php echo esc_html( $log->platform ); ?></td>
              <td style="color:var(--g500)"><?php echo esc_html( $log->event_name ); ?></td>
              <td>
                <?php if ( $log->success ) : ?>
                <span class="knk-badge knk-badge-green"><?php echo (int) $log->status_code; ?> OK</span>
                <?php else : ?>
                <span class="knk-badge knk-badge-red"><?php echo (int) $log->status_code; ?> Fail</span>
                <?php endif; ?>
              </td>
              <td style="color:var(--g400);font-size:11px"><?php echo esc_html( substr( $log->created_at, 0, 16 ) ); ?></td>
            </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
