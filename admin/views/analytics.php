...
<div class="knk-ph">
  <div class="knk-ph-left">
    <h1><i class="fa-solid fa-chart-bar"></i> Analitik</h1>
    <p style="margin:2px 0 0;color:var(--g500);font-size:13px;">Periode: <?php echo esc_html( $range_label ); ?></p>
  </div>
  <div class="knk-ph-right" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <form method="GET" id="knkAnalyticsForm" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
      <input type="hidden" name="page" value="konektor-analytics">
      <select name="days" id="knkDaysSelect" class="knk-input" style="width:130px;" onchange="knkOnDaysChange(this.value)">
        <?php foreach ( [7, 14, 30, 60, 90] as $d ) : ?>
          <option value="<?php echo $d; ?>" <?php echo ( ! $is_custom && $days === $d ) ? 'selected' : ''; ?>><?php echo $d; ?> Hari</option>
        <?php endforeach; ?>
        <option value="custom" <?php echo $is_custom ? 'selected' : ''; ?>>Kustom...</option>
      </select>
      <div id="knkCustomRange" style="display:<?php echo $is_custom ? 'flex' : 'none'; ?>;gap:6px;align-items:center;">
        <input type="date" name="date_from" id="knkDateFrom" class="knk-input" style="width:145px;"
               value="<?php echo $is_custom ? esc_attr( $date_from ) : ''; ?>" max="<?php echo date( 'Y-m-d' ); ?>">
        <span style="color:var(--g400);">—</span>
        <input type="date" name="date_to" id="knkDateTo" class="knk-input" style="width:145px;"
               value="<?php echo $is_custom ? esc_attr( $date_to ) : ''; ?>" max="<?php echo date( 'Y-m-d' ); ?>">
      </div>
      <button type="submit" class="knk-btn knk-btn-primary"><i class="fa-solid fa-filter"></i> Terapkan</button>
    </form>
...
<script>
function knkOnDaysChange(val) {
  var cr = document.getElementById('knkCustomRange');
  if (val === 'custom') {
    cr.style.display = 'flex';
    var df = document.getElementById('knkDateFrom');
    var dt = document.getElementById('knkDateTo');
    ...
    document.getElementById('knkDateFrom').value = '';
    document.getElementById('knkDateTo').value   = '';
  }
}
document.getElementById('knkAnalyticsForm').addEventListener('submit', function() {
  if (document.getElementById('knkDaysSelect').value !== 'custom') {
    document.getElementById('knkDateFrom').value = '';
    document.getElementById('knkDateTo').value   = '';
  }
});
...
<div class="knk-card" style="margin-top:4px">
  <div class="knk-card-head"><span class="knk-card-title"><i class="fa-solid fa-chart-line"></i> Lead per Hari</span><span style="font-size:12px;color:var(--g500);margin-left:8px;"><?php echo esc_html( $range_label ); ?></span></div>
...
<script>
var konektorDailyData = <?php echo wp_json_encode(array_map(function($r) { return ['date'=>$r->date,'leads'=>(int)$r->leads]; }, $daily)); ?>;
</script>
