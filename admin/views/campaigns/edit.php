...
function knk_tpl_thumb($val) {
  $cfg = [
    ...
    'classic_flat' => ['#FF5000','#b7d09a','#ccc'],
...
function knk_form_preview($cfg, $campaign) {
  ...
  // Pixel badges
  $px_cfg = ($campaign && $campaign->pixel_config) ? json_decode($campaign->pixel_config, true) : [];
  $px_badges = [];
  // Backward compat: if 'enabled' key doesn't exist, treat as enabled (old campaigns)
  $meta_on = !empty($px_cfg['meta']['pixel_id']) && (!array_key_exists('enabled', $px_cfg['meta'] ?? []) || !empty($px_cfg['meta']['enabled']));
  $tiktok_on = !empty($px_cfg['tiktok']['pixel_id']) && (!array_key_exists('enabled', $px_cfg['tiktok'] ?? []) || !empty($px_cfg['tiktok']['enabled']));
  $google_on = (!empty($px_cfg['google']['conversion_id']) || !empty($px_cfg['google']['gtm_id']) || !empty($px_cfg['google']['ga4_id'])) && (!array_key_exists('enabled', $px_cfg['google'] ?? []) || !empty($px_cfg['google']['enabled']));
  $snack_on = !empty($px_cfg['snack']['pixel_id']) && (!array_key_exists('enabled', $px_cfg['snack'] ?? []) || !empty($px_cfg['snack']['enabled']));
  if ($meta_on)   $px_badges[] = '<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#1877f2;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Meta</span>';
  if ($tiktok_on) $px_badges[] = '<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#010101;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>TikTok</span>';
  if ($google_on) $px_badges[] = '<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#4285f4;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Google</span>';
  if ($snack_on)  $px_badges[] = '<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#e4372c;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Snack</span>';
  $px_bar = $px_badges ? '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">' . implode('', $px_badges) . '</div>' : '';
...
$op_data = array_map(function($o) { return ['id' => (int)$o->id, 'name' => $o->name, 'type' => $o->type]; }, $operators ?? []);
    ...
    <div class="knk-card-head">
      ...
      <div class="knk-card-head-tools">
        ...
        <select name="form_config[template]" id="form-tpl-select" class="knk-select knk-select-sm">
          <?php foreach (['modern'=>'Modern','classic'=>'Classic','classic_flat'=>'Classic Flat','minimal'=>'Minimal','card'=>'Card','gradient'=>'Gradient','rose'=>'Rose','forest'=>'Forest','sunset'=>'Sunset','ocean'=>'Ocean'] as $v => $l) : ?>
    ...
    <div class="knk-card-body">
      ...
      <div class="knk-tpl-bar">
        ...
        <div class="knk-tpl-thumb <?php echo ($form_cfg['template'] ?? 'modern') === $v ? 'active' : ''; ?>" data-tpl="<?php echo $v; ?>">
          ...
          <span class="knk-tpl-label"><?php echo $l; ?></span>
        </div>
        <?php endforeach; ?>
      </div>
  ...
  <div class="knk-card">
    ...
    <div class="knk-card-body">
      ...
      <div class="knk-pcontent active" data-ptab="meta">
        <div class="knk-sw-row" style="margin-bottom:14px;padding:10px 12px;background:var(--g50);border-radius:8px;">
          <div class="knk-sw-row-label"><strong>Aktifkan Meta Pixel</strong></div>
          <label class="knk-switch">
            <input type="checkbox" name="pixel_config[meta][enabled]" value="1" <?php checked(!empty($meta_cfg['enabled'])); ?> onchange="document.getElementById('meta-fields').style.opacity=this.checked?'1':'.5';document.getElementById('meta-fields').style.pointerEvents=this.checked?'':'none';">
            <span class="knk-switch-slider"></span>
          </label>
        </div>
        <div id="meta-fields" style="<?php echo empty($meta_cfg['enabled']) ? 'opacity:.5;pointer-events:none;' : ''; ?>">
        <div class="knk-g2">
          <div class="knk-field"><label class="knk-label">Pixel ID</label><input type="text" name="pixel_config[meta][pixel_id]" class="knk-input" value="<?php echo esc_attr($meta_cfg['pixel_id']??''); ?>" placeholder="123456789012345"></div>
        ...
        <div class="knk-g<?php echo $is_wa ? '2' : '3'; ?>">
          <div class="knk-field">
            ...
            <?php echo knk_evt_select('pixel_config[meta][page_load_event]', $meta_cfg['page_load_event']??'', $meta_events); ?>
          </div>
          <div class="knk-field">
            <label class="knk-label"><?php echo $is_wa ? 'Event WA Click' : 'Event Form Submit'; ?></label>
            <?php echo knk_evt_select('pixel_config[meta][form_submit_event]', $meta_cfg['form_submit_event']??'', $meta_events); ?>
          </div>
          <?php if (!$is_wa) : ?>
          <div class="knk-field">
            <label class="knk-label">Event Thanks Page</label>
            <?php echo knk_evt_select('pixel_config[meta][thanks_page_event]', $meta_cfg['thanks_page_event']??'', $meta_events); ?>
          </div>
          <?php endif; ?>
        </div>
        </div>
      ...
      <div class="knk-pcontent" data-ptab="google">
        <div class="knk-sw-row" style="margin-bottom:14px;padding:10px 12px;background:var(--g50);border-radius:8px;">
          <div class="knk-sw-row-label"><strong>Aktifkan Google Pixel</strong></div>
          <label class="knk-switch">
            <input type="checkbox" name="pixel_config[google][enabled]" value="1" <?php checked(!empty($google_cfg['enabled'])); ?> onchange="document.getElementById('google-fields').style.opacity=this.checked?'1':'.5';document.getElementById('google-fields').style.pointerEvents=this.checked?'':'none';">
            <span class="knk-switch-slider"></span>
          </label>
        </div>
        <div id="google-fields" style="<?php echo empty($google_cfg['enabled']) ? 'opacity:.5;pointer-events:none;' : ''; ?>">
        <div class="knk-g2">
      ...
      <div class="knk-pcontent" data-ptab="tiktok">
        <div class="knk-sw-row" style="margin-bottom:14px;padding:10px 12px;background:var(--g50);border-radius:8px;">
          <div class="knk-sw-row-label"><strong>Aktifkan TikTok Pixel</strong></div>
          <label class="knk-switch">
            <input type="checkbox" name="pixel_config[tiktok][enabled]" value="1" <?php checked(!empty($tiktok_cfg['enabled'])); ?> onchange="document.getElementById('tiktok-fields').style.opacity=this.checked?'1':'.5';document.getElementById('tiktok-fields').style.pointerEvents=this.checked?'':'none';">
            <span class="knk-switch-slider"></span>
          </label>
        </div>
        <div id="tiktok-fields" style="<?php echo empty($tiktok_cfg['enabled']) ? 'opacity:.5;pointer-events:none;' : ''; ?>">
        ...
        <div class="knk-g<?php echo $is_wa ? '2' : '3'; ?>">
          <div class="knk-field">
            <label class="knk-label">Event Page Load</label>
            <?php echo knk_evt_select('pixel_config[tiktok][page_load_event]', $tiktok_cfg['page_load_event']??'', $tiktok_events); ?>
          </div>
          <div class="knk-field">
            <label class="knk-label"><?php echo $is_wa ? 'Event WA Click' : 'Event Form Submit'; ?></label>
            <?php echo knk_evt_select('pixel_config[tiktok][form_submit_event]', $tiktok_cfg['form_submit_event']??'', $tiktok_events); ?>
          </div>
          <?php if (!$is_wa) : ?>
          <div class="knk-field">
            <label class="knk-label">Event Thanks Page</label>
            <?php echo knk_evt_select('pixel_config[tiktok][thanks_page_event]', $tiktok_cfg['thanks_page_event']??'', $tiktok_events); ?>
          </div>
          <?php endif; ?>
        </div>
        </div>
      ...
      <div class="knk-pcontent" data-ptab="snack">
        <div class="knk-sw-row" style="margin-bottom:14px;padding:10px 12px;background:var(--g50);border-radius:8px;">
          <div class="knk-sw-row-label"><strong>Aktifkan SnackVideo Pixel</strong></div>
          <label class="knk-switch">
            <input type="checkbox" name="pixel_config[snack][enabled]" value="1" <?php checked(!empty($snack_cfg['enabled'])); ?> onchange="document.getElementById('snack-fields').style.opacity=this.checked?'1':'.5';document.getElementById('snack-fields').style.pointerEvents=this.checked?'':'none';">
            <span class="knk-switch-slider"></span>
          </label>
        </div>
        <div id="snack-fields" style="<?php echo empty($snack_cfg['enabled']) ? 'opacity:.5;pointer-events:none;' : ''; ?>">
        ...
        <div class="knk-g<?php echo $is_wa ? '2' : '3'; ?>">
          <div class="knk-field">
            <label class="knk-label">Event Page Load</label>
            <?php echo knk_snack_select('pixel_config[snack][page_load_event]', $snack_cfg['page_load_event']??'', $snack_events); ?>
          </div>
          <div class="knk-field">
            <label class="knk-label"><?php echo $is_wa ? 'Event WA Click' : 'Event Form Submit'; ?></label>
            <?php echo knk_snack_select('pixel_config[snack][form_submit_event]', $snack_cfg['form_submit_event']??'', $snack_events); ?>
          </div>
          <?php if (!$is_wa) : ?>
          <div class="knk-field">
            <label class="knk-label">Event Thanks Page</label>
            <?php echo knk_snack_select('pixel_config[snack][thanks_page_event]', $snack_cfg['thanks_page_event']??'', $snack_events); ?>
          </div>
          <?php endif; ?>
        </div>
...
<div class="knk-cols-side">
  ...
  <!-- Pixel Code Preview -->
  <div class="knk-card">
    <div class="knk-card-head"><h2><i class="fa-solid fa-code"></i> Preview Kode Pixel</h2></div>
    <div class="knk-card-body">
      <p style="font-size:12px;color:var(--g500);margin:0 0 10px">Kode pixel browser-side yang akan diinject ke halaman:</p>
      <?php
      $preview_event = 'page_load';
      $has_pixel = false;
      $meta_script = $campaign ? Konektor_Meta::get_pixel_script($campaign, $preview_event) : '';
      $tiktok_script = $campaign ? Konektor_Tiktok::get_script($campaign, $preview_event) : '';
      $google_script = $campaign ? Konektor_Google::get_script($campaign, $preview_event) : '';
      $snack_script  = $campaign ? Konektor_Snack::get_script($campaign) : '';

      if ($meta_script) {
          $has_pixel = true;
          echo '<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:600;color:#1877f2;margin-bottom:4px;">Meta / Facebook Pixel</div><pre style="background:var(--g50);padding:10px 12px;border-radius:6px;font-size:11px;overflow-x:auto;white-space:pre-wrap;max-height:160px;overflow-y:auto;margin:0;">' . esc_html(trim($meta_script)) . '</pre></div>';
      }
      if ($tiktok_script) {
          $has_pixel = true;
          echo '<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:600;color:#010101;margin-bottom:4px;">TikTok Pixel</div><pre style="background:var(--g50);padding:10px 12px;border-radius:6px;font-size:11px;overflow-x:auto;white-space:pre-wrap;max-height:160px;overflow-y:auto;margin:0;">' . esc_html(trim($tiktok_script)) . '</pre></div>';
      }
      if ($google_script) {
          $has_pixel = true;
          echo '<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:600;color:#4285f4;margin-bottom:4px;">Google Ads / GTM / GA4</div><pre style="background:var(--g50);padding:10px 12px;border-radius:6px;font-size:11px;overflow-x:auto;white-space:pre-wrap;max-height:160px;overflow-y:auto;margin:0;">' . esc_html(trim($google_script)) . '</pre></div>';
      }
      if ($snack_script) {
          $has_pixel = true;
          echo '<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:600;color:#e4372c;margin-bottom:4px;">SnackVideo / Kwai Pixel</div><pre style="background:var(--g50);padding:10px 12px;border-radius:6px;font-size:11px;overflow-x:auto;white-space:pre-wrap;max-height:160px;overflow-y:auto;margin:0;">' . esc_html(trim($snack_script)) . '</pre></div>';
      }
      if (!$has_pixel) {
          echo '<p style="text-align:center;color:var(--g400);font-size:13px;padding:16px;">Tidak ada pixel yang dikonfigurasi.</p>';
      }
      ?>
    </div>
  </div>
